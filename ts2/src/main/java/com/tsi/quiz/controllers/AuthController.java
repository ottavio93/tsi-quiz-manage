...
import com.tsi.quiz.security.jwt.JwtUtils;
import com.tsi.quiz.security.services.UserDetailsImpl;
...
if (verita ){
	...
	@PostMapping("/signupadad")
	public ResponseEntity<?> registerad(@Valid @RequestBody SignupRequest signUpRequest) {
		if (userRepository.existsByUsername(signUpRequest.getUsername())) {
			return ResponseEntity
					.badRequest()
					.body(new MessageResponse("Error: Username is already taken!"));
